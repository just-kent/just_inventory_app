				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("ItemCode") === oObject.ItemCode;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].ItemCode;
										} else {
											sCurrentId = aItemsNotInTheList[0].ItemCode;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ITEM_STOCK", {
											ItemCode : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("ItemCode"),
							title: oBindingContext.getProperty("ItemName")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "ITEM_STOCK (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'ITEM_STOCK (0)'");
							},
					...
					theListShouldBeSortedAscendingOnUnitNumber : function () {
						return this.theListShouldBeSortedAscendingOnField("CC_STOCK_MOV_BAL");
					},

					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("ItemName");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all ITEM_STOCK to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "ITEM_STOCK (" + iExpectedLength + ")"}),
									success : function () {
